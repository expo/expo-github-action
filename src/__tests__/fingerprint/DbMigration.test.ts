...
describe('DbMigrationCoordinator', () => {
  ...
  describe('migration steps', () => {
    it('should handle migration from version 0 to 1', async () => {
      // Create a version 0 database with fingerprint table but no github_artifact_id column
      ...
      // Check that github_artifact_id column was added
      const tableInfo = await db.allAsync<{ name: string }>('PRAGMA table_info(fingerprint)');
      const columnNames = tableInfo.map(col => col.name);
      expect(columnNames).toContain('github_artifact_id');
      expect(columnNames).toContain('platform');

      // Check that github_artifacts table was created
